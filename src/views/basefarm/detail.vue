...
<script>
import {detail} from '@/api/basefarm'
...
export default {
    components:{
      ...
      created(){
           ...
           detail(id).then(res=>{
                let {photos,trendsInfo}=res.data;
                if(this.userinfo){
                      this.load=res.data
                }else{
                    let livestockCount=res.data.livestockCount
                    livestockCount.forEach(item=>{
                         item.count="***"
                    })
                    let poultryCount=res.data.poultryCount
                    poultryCount.forEach(item=>{
                         item.count='***'
                    })
                    let insured=res.data.insured
                    insured.forEach(item=>{
                         item.count='***'
                    })
                    let poorBreed=res.data.poorBreed
